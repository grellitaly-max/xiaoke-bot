...
def generate_message(hours_silent):
    prompt = f"你是阿筠的德语学习提醒助手，名字叫小克。阿筠已经 {hours_silent:.0f} 小时没有学德语了。请用中文写一条催她学习的消息，语气可以俏皮、温柔、或者稍微有点撒娇，不要太严肃。不超过100字。直接输出消息内容，不要加任何前缀。"
    response = client.models.generate_content(model="gemini-2.0-flash", contents=prompt)
    return response.text.strip()

def generate_daily_message():
    prompt = "你是阿筠的德语学习提醒助手，名字叫小克。请用中文写一条早上的德语学习提醒，语气温暖有趣，可以加一句简单的德语鼓励。不超过100字。直接输出消息内容，不要加任何前缀。"
    response = client.models.generate_content(model="gemini-2.0-flash", contents=prompt)
    return response.text.strip()
